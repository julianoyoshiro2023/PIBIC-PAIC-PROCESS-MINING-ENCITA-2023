...
class ModelGeneratorNS:
	...
	def __init__(self, n, s, rng, first_step = -1, last_step = -1, failure_rate=0.1, buffer_size=1, production_rate=1, production_level="constant", 
	...
	def generate_graph(self):
		...
		while (node_index < self.n):
			...
			if self.first_step != -1 and self.last_step != -1:
				...
				r_uniform = self.rng.uniform(1,self.p_steps-1,1)
			elif self.first_step != -1:
				# draw a sample from the uniform distribution
				r_uniform = self.rng.uniform(1,self.p_steps,1)
			elif self.last_step != -1:
				r_uniform = self.rng.uniform(0,self.p_steps-1,1)
			else:
				# draw a sample from the uniform distribution
				r_uniform = self.rng.uniform(0,self.p_steps,1)
